...
function F = FieldCrossMovie(x, y, z, Exr, Eyr, Ezr, Exp, Eyp, Ezp, paramVals, options) %movieName)
arguments
...
F = FieldCrossRender(x, y, Exr(:,:,1),Eyr(:,:,1),Ezr(:,:,1), Exp(:,:,1),Eyp(:,:,1),Ezp(:,:,1), options.renderParams{:});
...
% non-interactive: print series to video file
	...
	for n = 1:length(paramVals)
		fprintf("n=%d\n",n);
		F.Render(x, y, Exr(:,:,n),Eyr(:,:,n),Ezr(:,:,n),Exp(:,:,n),Eyp(:,:,n),Ezp(:,:,n),paramVals(n));
